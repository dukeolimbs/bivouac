...
export const SETTING_GROUPS: readonly SettingGroup[] = [
  ...
  },
  {
    ...
    keys: [SETTINGS.castBarFont, SETTINGS.castBarFontCustom, SETTINGS.castBarFontSize],
  },
  {
    label: "BIVOUAC.Settings.Groups.CastBarStats",
    keys: [SETTINGS.castStatAC, SETTINGS.castStatPP, SETTINGS.castStatHP, SETTINGS.castStatInv],
  },
];
...
function markWorldScope(row: HTMLElement, key: string): void {
  const setting = game.settings.settings.get(`${MODULE_ID}.${key}`) as { scope?: string } | undefined;
...
function syncSectionVisibility(container: HTMLElement): void {
  for (const section of container.querySelectorAll<HTMLElement>("fieldset.bivouac-settings-group")) {
    const anyVisible = Array.from(section.querySelectorAll<HTMLElement>(".form-group")).some((row) => !row.hidden);
...
export function decorateSettingsForm(root: HTMLElement): void {
  ...
  const first = planned
    .flatMap((p) => p.rows)
    .reduce((a, b) => (a.compareDocumentPosition(b) & Node.DOCUMENT_POSITION_PRECEDING ? b : a));
  ...
  searchObserver.observe(container, { attributes: true, attributeFilter: ["hidden"], subtree: true });
}
